(ns autogpt_forge.main
  "Kotodama WASM entrypoint for the autogpt_forge clean-room actor (L4) — Clojure port.
  ...
  facts (`autogpt_forge.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "autogpt_forge")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"autogpt_forge.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "autogpt_forge-compat" :tier tier :entities entities} 200])
...
(defn start! [] :autogpt_forge-compat/ready)
